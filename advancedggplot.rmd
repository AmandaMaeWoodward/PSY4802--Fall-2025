...
today we're simulating data on... number of boba shops per state university 
```{r}
set.seed(137)
...
Main effect: effect of a predictor on the outcome on its own (ignoring the other predictors)
Interactions take other predictors into account
...
# checking data because something weird happened
```{r}
subset(bobaDat, bobaDat$schoolClimate=="cool" & bobaDat$studentInterest=="high") #could also group by and/or use filter in a pipe
```
turns out, not weird... the cool & high combo doesn't exist
...
using color 
```{r}
ggplot(bobaDat, aes(studentSize, boba, color= studentInterest))+ geom_point()+geom_smooth(method="lm")

```

using ggeffects package
```{r}
lm2<- lm(boba ~ studentInterest*studentSize  , data=bobaDat)
a2<- ggpredict(lm2)
plot(a2)
...
```
#### Practice (Challenge)
1. Create a shiny app that allows people to select a number between 1 and 37500. 
```{r}
ui <- fluidPage(
  sliderInput(inputId = "number", 
    label = "Pick a number", 
