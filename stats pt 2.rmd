...
```{r}
penguinLm<- lm(flipper_len_mm~species , data=penguins)
```
...
```{r}
penguinLm<- lm(flipper_len_mm~species, weights=speciesSize , data=penguins)
```
...
```{r}
penguinMod<- lm(body_mass_g ~ flipper_len_mm*bill_depth_mm, data=penguins)
```
