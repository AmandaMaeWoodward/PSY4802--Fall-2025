...
```{r}
ggplot(priceByColor, aes(color, price))+ geom_bar(stat="identity", fill="lightblue",alpha=.75)+ geom_errorbar(aes(ymin= price - se, ymax= price + se, width=.5 ))+theme_classic()+ geom_vline(xintercept= "G", color= "green")

ggplot(priceByColor, aes(color, price))+ geom_bar(stat="identity", fill="lightblue",alpha=.75)+ geom_errorbar(aes(ymin= price - se, ymax= price + se, width=.5 ))+theme_classic()+ geom_vline(xintercept= 2, color= "green") #better to explicitly code where the line should go
...
```{r}
ggplot(penguins, aes(species))+geom_bar(color="black", fill="orange") +theme_classic()+ xlab("species")+ ylab("frequency")+ ggtitle("Penguins")
```

```{r}
ggplot(penguins, aes(species))+geom_bar(color="black", fill="lightpink") +theme_classic()+labs(title="different species of penguins", x= "species", y="number of penguins") 
```
...
#stacked bar graph
ggplot(diamonds, aes(color, fill=cut))+ geom_bar(position=position_dodge())+ theme_classic() #categorical
...
```{r}
ggplot(diamonds, aes(carat, price))+ geom_point(color= "red", fill="lightblue") # fill doesn't work for scatteplots, only color
```
...
ggplot(mtcars, aes(cyl, gear))+geom_jitter()#only add this to other graphs to get a sense of variability 
```
...
```{r}
ggplot(penguins, aes(bill_length_mm, bill_depth_mm))+ geom_point(color= "lightpink", size=2, shape=10)
```
...
```{r}
ggplot(diamonds, aes(carat,price))+ geom_point()+ geom_smooth(method="lm", size= 1, color="orange", linetype="dashed")
```
...
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color="purple3")

```
3. Change the shape of the points. 
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color="purple3", shape=15)

```
4. Add a line of best fit to your scatter plot. 
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color="purple3", shape=15)+geom_smooth(method="lm", color="black")
...
```{r}
ggplot(diamonds, aes(color, price))+ geom_boxplot()+geom_jitter()
```
...
use fill = 
price histogram, how i can add color as another variable? 
```{r}
...
For scatter plots: 
color = 
for scatterplot of price, carat, how can I add cut as another variable?
```{r}
...
```{r}
ggplot(diamonds, aes(price, fill= color))+geom_histogram(position=position_dodge(), bins=15)+ scale_fill_manual(values=c("#D7F2BA", "#0D2818", "#BC4749", "#058C42", "#FEC196", "#044B7F", "#011627" ))
```
`scale_color_manual(values = #)`; 
```{r}
ggplot(diamonds, aes(price, carat, color=cut))+ geom_point()+ scale_color_manual(values=c("#D7F2BA", "#0D2818", "#BC4749", "#058C42", "#FEC196", "#044B7F", "#011627" ))
```
...
ggplot(diamonds, aes(price, carat, color=cut))+ geom_point()+scale_color_brewer(palette= "GnBu")+ theme_classic()
...
ggplot(diamonds, aes(price, carat, color= cut))+ geom_point()+scale_color_viridis(discrete= TRUE, option ="inferno")
ggplot(diamonds, aes(price, carat, color= table))+ geom_point()+scale_color_viridis(discrete= FALSE, option ="inferno")
```
...
```{r}
ggplot(penguins, aes(bill_length_mm, island))+ geom_point()
ggplot(penguins, aes(island, bill_length_mm))+ geom_boxplot()
```

2. In some way, include species as a third variable in this graph. 
```{r}
ggplot(penguins,aes(island, bill_length_mm, fill=species))+ geom_boxplot()
```
