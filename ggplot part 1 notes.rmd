...
pipeVersion<- penguins %>% 
  group_by(species) %>% 
  summarise(meanFlipper= mean(flipper_length_mm), stdDev= sd(flipper_length_mm), stdError= sd(flipper_length_mm)/sqrt(nrow(penguins)), lowercI = (mean(flipper_length_mm) - .95*sd(flipper_length_mm))/sqrt(nrow(penguins)), upperCI= (mean(flipper_length_mm) + .95*sd(flipper_length_mm))/sqrt(nrow(penguins)))  
