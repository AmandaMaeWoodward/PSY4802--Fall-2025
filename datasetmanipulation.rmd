...
```{r}
aggressionDat <- read.csv("~/Desktop/PSY4802- Fall 2025/PSY4802-AggressionSampleData.csv") # can be used; but it isn't super reproducible. 

library(readr)
aggressionDat <- read_csv("PSY4802-AggressionSampleData.csv") #more reproducible
```
getting/setting wd
```{r}
getwd()
setwd(dir = "~/Downloads/") # setwd() lets me change my working directory. switched to downloads (from: )
...
```
```{r}
#can add on to the data manipulation pipe to do other things like summarize
billboard %>% 
  pivot_longer(cols=4:79, names_to="week", values_to= "score") %>%
  summarize(mean=mean(score, na.rm=TRUE)) # provide a summary where you create an object called mean and save the mean of the score column 
```
...
```{r}
aggressionLongDat %>% pivot_wider(id_cols= c("subjNo", "ageGroup","parentReportGender", "condition"), names_from="timePoint", values_from= "aggressionScore")
```
...
```{r}
aggressionLongDat2<- spread(aggressionLongDat, key = "timePoint", value= "aggressionScore")
```
...
```{r}
chickWeightWide<- ChickWeight%>%
  pivot_wider(id_cols = c("Chick", "Diet"), names_from="Time", values_from="weight")
```
#lessons learned
pivot_wider(ChickWeight, id_cols = c("Chick", "Diet", "Time", "weight"),names_from="Time", values_from="weight") #won't work, can only use a column name once

a<-pivot_wider(ChickWeight, id_cols= (1:2), names_from="Time", values_from="weight") #same column can't be called twice with different methods
#explicitly call columns to avoid strange outcomes... 
#extra credit to whoever figures this out


3. Convert from wide to long. 
```{r}
chickLong<- pivot_longer(chickWeightWide, cols= c(colnames(chickWeightWide)[3:14]), names_to="Time", values_to="weight")
# contains ~20 more rows than the original 

chickLong2<- na.omit(chickLong)
ifelse(is.na(chickWeight$weight)==TRUE)
#is.na will help you remove specific nas from dataset; combine with indexing or subsetting
```
...
2) identify the type of data (wide/long)
long
...
```{r}
data("ChickWeight")
chickThree<- ChickWeight %>% filter(Chick==3) %>% pivot_wider(id_cols=c("Chick", "Diet"), names_from ="Time" ,values_from= "weight")
```
...
```{r}
#find an anxiety scale to find range of possible scores
#Hamilton anxiety rating scale (0 no anxiety; <17 mild;18-24- mild/moderate, 25-30- moderate/severe ;30- 56- severe)
...
```{r}
simulationEFScores<- sample(size=48, x= 0:16, replace=TRUE)
simulationEFScores
learnOutThree<- data.frame(subjNo= aggressionDat$subjNo, simulationEFScores)

```

2. Join your dataset in #1 to the original Aggression dataset. 
```{r}
execAggressionDat<- left_join(aggressionDat, learnOutThree)
```
...
```{r}
class(aggressionDat$parentReportGender) #check to see if factor
...
```{r}
aggressionDat$condition<-as.factor(aggressionDat$condition)
aggressionDat$condition<- relevel(aggressionDat$condition, ref= "Inclusion")
aggressionDat$condition
```
...
```{r}
updatedStarwars<-starwars %>% mutate(meters= height/100)
```
