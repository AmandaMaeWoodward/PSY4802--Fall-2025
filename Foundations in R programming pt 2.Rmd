...
tolower(colnames(mtcars)[4]) #this isn't saving it 
```
...
levels(mtcars$transmission)# gives us the levels of a factor

#preserving the value rather than the category position
mtcars$transmission3<-  as.numeric(levels(mtcars$transmission)[mtcars$transmission])
mtcars %>% select(transmission, transmission3) #compare before saving

#mtcars$am<- as.factor(mtcars$am) #check your column names
```
...
# c("my favorite food is", "dumplings") could work, but annoying
```
...
```{r}
columnMean<- function(number1, number2){
...
vectNum<- 10:100
number<- NA
for(i in 1:length(vectNum)){
  number[i]<-vectNum[i]*4 #create a vector to save the output
}

for(placeholder in 1:length(vectNum)){
  print(vectNum[placeholder]*4) #output the results to the console
}
number
